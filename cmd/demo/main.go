...
// demo11_v2RunAPI 演示 v2 的 Run 子句式 API：Go 惯用法（error 返回值 + errors.As 穿透 + Cleanup 总是执行）
func demo11_v2RunAPI() {
	fmt.Println("\n=== Demo 11: v2 Run 子句式 API（Go 惯用法）===")

	// 1. 基本形态：if err != nil
	err := gotrycatch.Run(func() {
		gotrycatch.Throw(trycatcherrors.NewValidationError("age", "must be positive", 1004))
	},
		gotrycatch.OnAs(func(e trycatcherrors.ValidationError) {
			fmt.Printf("✓ OnAs 捕获 ValidationError: field=%s code=%d\n", e.Field, e.Code)
		}),
	)
	fmt.Printf("  处理后 err == nil: %v\n", err == nil)

	// 2. 包装错误穿透：fmt.Errorf %w 包装后仍能命中
	err = gotrycatch.Run(func() {
		gotrycatch.Throw(fmt.Errorf("save user failed: %w", trycatcherrors.NewDatabaseError("INSERT", "users", errors.New("conn refused"))))
	},
		gotrycatch.OnAs(func(e trycatcherrors.DatabaseError) {
			fmt.Printf("✓ OnAs 穿透 %%w 包装命中 DatabaseError: %s on %s\n", e.Operation, e.Table)
		}),
	)
	fmt.Printf("  处理后 err == nil: %v\n", err == nil)

	// 3. 未处理错误 → error 返回，支持 errors.Is/As 链路
	err = gotrycatch.Run(func() { gotrycatch.Throw("mystery failure") })
	fmt.Printf("  未处理错误: %v (类型 %T)\n", err, err)

	// 4. Run1 带返回值 + Cleanup 兜底清理
	var closed bool
	value, err := gotrycatch.Run1(func() int {
		gotrycatch.Throw(trycatcherrors.NewRateLimitError("/api", 100, 120, 30))
		return 0
	},
		gotrycatch.Cleanup(func() { closed = true }),
	)
	fmt.Printf("  Run1 结果: value=%d err=%v cleanup执行=%v\n", value, err != nil, closed)

	// 5. Err() 桥接：TryBlock → error
	tb := gotrycatch.Try(func() { gotrycatch.Throw(trycatcherrors.NewConfigError("k", "v", "bad")) })
...
func main() {
	...
	fmt.Println("\n🎉 所有Demo执行完成！")
	fmt.Println("\n功能摘要:")
	fmt.Println("  v2 惯用法: Run/Run1 子句式, On/OnAs/Any/Cleanup, CatchAs, Err() 桥接, PanicError")
	fmt.Println("  1. TryBlock 状态查询: GetError(), HasError(), IsHandled(), String(), GetErrorType(), CanonicalErrorType()")
	...
	fmt.Println("  8. 新增错误类型: ConfigError, AuthError, RateLimitError（errtypes 包）")
}
